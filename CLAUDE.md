# Equipment & Fighting Styles (acks-equipment)
...
- `scripts/` — ESM runtime, entry `scripts/module.mjs`; `templates/` — .hbs;
  `styles/`; `lang/en.json` — flat i18n keys prefixed `ACKS-EQUIPMENT.`
...
- `tools/` — dev harness. `build-packs.mjs` and `validate.mjs` are **synced
  ...
  module-owned.
- Canonical ACKS II rules extract: `C:\Proj\acks-rules\acks-equipment\RULES.md`
...
- Foundry dev install (junction, not copy):
  `New-Item -ItemType Junction -Path "$env:LOCALAPPDATA\FoundryVTT\Data\modules\acks-equipment" -Target "C:\Proj\acks-equipment"`
## Live testing
...
5. Confirm publication with BOUNDED polls — **never `gh run watch`, it hangs**:
   ...
   verify `https://github.com/NocTempre/acks-equipment/releases/latest/download/module.json`
...
- Namespacing (validate-enforced): globals/custom hooks/HB helpers start with
  ...
  `flags["acks-equipment"].idPrefix` key; lang keys with `ACKS-EQUIPMENT.`;
  CSS classes with `acks-equipment-`.
- Design doctrine: **reuse → extend → enhance → invent** — reuse core system
  documents; extend only via `flags["acks-equipment"]`; enhance with alternate
